...
fn main() {
    if let Err(_) = set_database() {
        print!("Error! something gets wrong");
        return;
    }
    ...
    for stream in listener.incoming() {
        match stream {
            ...
            Err(e) => {
                print!("Not able to connect the server {}", e);
            }
        }
    }
}
...
fn handle_client(mut stream: TcpStream){
    ...
    match stream.read(&mut buffer){
        ...
        Err(e) => eprint!("Unable to read stream : {}",e),
    }
    ...
    match (get_user_request_body(request),Client::connect(DB_URL, NoTls)) {
        (Ok(user), Ok(mut client)) => {
            ...
            match client.query_one("SELECT id, name, email FROM users WHERE id = $1", &[&user_id]) {
                ...
                Err(_) =>
                    (INTERNAL_ERROR.to_string(),"Failed to reterive created user".to_string())
...
fn handle_get_request(request: &str) -> (String, String) {
    match (get_id(&request),parse::<i32>(), Client::connect(DB_URL, NoTls)) {
        ...
        _ => (INTERNAL_ERROR.to_string(),"Internal eoor".to_string()),
        
    }
}

fn handle_get_all_request(reuest: &str) -> (String,String) {
...
fn handle_put_request(request: &str) -> (String, String) {
    match ( 
        ...
        (Ok(id), Ok(user),Ok(mut client)) => {
            ...
            (OK_RESPONSE.to_string(),"User updated succesfully".to_string())
        }
        _ => (INTERNAL_ERROR.to_string(),"Internal error".to_string()),
...
fn handle_delete_request(request: &str) -> (String,String) {
    match (get_id(&request).parse::<i32>(), Client::connect(DB_URL, NoTls)) {
        (Ok(id),Ok(mut client)) => {
            let row_affected = client.execute("DELETE FROM user WHERE id = $1", &[&id]).unwrap();
